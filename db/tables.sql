...
CREATE TABLE `price`(
      ...
      FOREIGN KEY (id_seller,id_productModel) REFERENCES    
);
...
CREATE TABLE `Có_Đơn hàng_nhà bán_mẫu sp`
...
CREATE TABLE `product_in_receipt`( --sản phẩm trong hóa đơn
      ...
      FOREIGN (STT) REFERENCES product(STT),
      FOREIGN (id_receipt) REFERENCES receipt(id)
);
